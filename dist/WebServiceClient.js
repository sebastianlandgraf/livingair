...
import { isValidStringLen, parseVarName, dataToByteArray, getTypeAndFormat, } from './helper.js';
...
export class WebServiceClient {
    ...
    constructor(service) {
        ...
        if ((service.amsPort ?? 0) < 801 || (service.amsPort ?? 0) > 891) {
            log('TAME library error: AMS Port Number (' +
                service.amsPort +
        ...
        //Username/password
        if (typeof service.serviceUser === 'string' &&
        ...
        if (service.dontFetchSymbols === true) {
            ...
            if (typeof service.configFileUrl != 'string' ||
                ...
                try {
                    ...
                    log("TAME library error: Could'nt fetch the symbol information from the PLC:" +
                        e);
                    return;
                }
            }
        ...
        if (this.symTableOk &&
            ...
            do {
                ...
                if (this.dataTypeTable[dataType].subItems[typeArray[i]].pointer === true) {
                    log('TAME library error: PLC variable ' +
                        [typeArray[i]] +
            ...
            try {
                ...
                if (itemInfo.dataTypeArrIdx[i] !== undefined &&
                    itemInfo.type === 'ARRAY') {
                    itemInfo.type =
                        this.dataTypeTable[dataType].subItems[typeArray[i]].arrayDataType;
                    itemInfo.size =
            ...
            try {
                ...
                if (itemInfo.symbolNameArrIdx !== undefined &&
                    itemInfo.type === 'ARRAY') {
    ...
    createRequest(adsReq) {
        ...
        adsReq.send = () => {
            ...
            if (typeof adsReq.reqDescr.id === 'number' &&
                ...
                log('TAME library warning: Request dropped (last request with ID ' +
                    adsReq.reqDescr.id +
                    ' not finished!)');
    ...
    parseReadReq(adsReq) {
        ...
        let response, arrType = [], strAddr = 0, item, dataString, dataSubString, data, strlen, len, plen, mod, type, format, startaddr;
        try {
            ...
            for (let idx = 0, listlen = itemList.length; idx < listlen; idx++) {
                ...
                else if (adsReq.reqDescr.calcAlignment === true &&
                    plen > 1 &&
                    type !== 'EndStruct' &&
                    type !== 'STRING' &&
                    strAddr > 0) {
    ...
    parseSumReadReq(adsReq) {
        let response, itemList = adsReq.reqDescr.items, strAddr = 0, subStrAddr = 0, dataObj = window, vlenMax = 0, item = {}, dataString, dataSubString = '', data, len = 0, type = '', format = '', listlen, errorCode, jvar = '';
        let i = null;
        let arrayLength;
        let itemSize;
        let itemInfo;
    ...
    parseSumWriteReq(adsReq) {
        let response, itemList = adsReq.reqDescr.items, strAddr = 0, item, dataString, dataSubString, data, listlen, errorCode;
    ...
    createArrayDescriptor(method, type, args) {
        ...
        if (method === 'Write' && typeof args.val === 'object') {
            ...
            log('TAME library error: No data object for this ' +
                method +
                '-Request defined!');
        }
     ...
     */
    writeReq(reqDescr) {
        let itemList = reqDescr.items, adsReq = {}, pData = [], arrType = [], bytes = [], type, format, len, pcount, mod, item, i;
        ...
        for (let idx = 0, listlen = itemList.length; idx < listlen; idx++) {
            ...
            if (isNaN(plc.plcTypeLen[type])) {
                log('TAME library error: Could not get the length of the data type: ' +
                    type);
            ...
            len = plc.plcTypeLen[type] < this.alignment ? plc.plcTypeLen[type] : this.alignment;
            if (reqDescr.calcAlignment === true &&
                len > 1 &&
                type !== 'STRING' &&
                type !== 'EndStruct' &&
                pData.length > 0) {
     ...
     */
    readReq(reqDescr) {
        let adsReq = {}, itemList = reqDescr.items, arrType = [], item, format, type, listlen, mod, vlen, strlen, startaddr;
     ...
     */
    sumReadReq(reqDescr) {
        let adsReq = {}, itemList = reqDescr.items, reqBuffer = [], bytes = [], listlen = itemList.length, dummy = {}, format, item, len, pwrData, itemInfo;
    ...
    sumWriteReq(reqDescr) {
        let adsReq = {}, itemList = reqDescr.items, reqBuffer = [], bytes = [], listlen = itemList.length, dummy = {}, vlenMax = 0, type, format, item, len, pwrData, i, k, arrayLength, pcount, itemInfo;
        /**
         ...
         */
        const parseStruct = () => {
            let j, defArr, lenArrElem, lastDefArr, mod, elem, subBuffer = [];
            /**
            ...
            for (elem in item.def) {
                if (item.def.hasOwnProperty(elem)) {
                    try {
                        ...
                        log('TAME library error: Could not set values for a structure in SumWriteReq: ' +
                            e);
                        log(item);
                    }
                }
    ...
    setSymbolsFromJSON(jstr) {
        if (typeof JSON !== 'object') {
            ...
            try {
                ...
                log('TAME library error: Could not create the Symbol Table from JSON:' +
                    e);
                return;
            }
    ...
    getDataTypesAsJSON() {
        ...
        if (typeof JSON !== 'object') {
            ...
            try {
                ...
                log('TAME library error: Could not convert the Data Type Table to JSON:' +
                    e);
            }
        }
    }
    ...
    setDataTypesFromJSON(jstr) {
        if (typeof JSON !== 'object') {
            ...
            try {
                ...
                log('TAME library error: Could not create the Data Type Table from JSON:' +
                    e);
                return;
            }
    ...
    parseResponse(adsReq) {
        ...
        try {
            ...
            log('TAME library error: Message from server:  ' +
                errorText +
                ' (' +
                errorCode +
                ')');
     ...
     */
    parseUpload(adsReq) {
        let response, strAddr = 0, igOffs = 4, ioOffs = 8, sizeOffs = 12, nameOffs = 30, dataString, dataSubString, cnt, infoLen, nameAndType, typeArr, arrayLength, type, elem;
        try {
            response = adsReq.xmlHttpReq.responseXML.documentElement;
            dataString = decodeBase64(response.getElementsByTagName('ppData')[0].firstChild.data);
            ...
            log('TAME library error: Parsing of uploaded symbol information failed:' +
                e);
            return;
        }
    }
     ...
     */
    getConfigFile() {
        let xmlHttpReq = createXMLHttpReq(), symbolArray = [], configFile, name, allSymbols, typeArr, arrayLength, type, elem, tcVersion;
        ...
        if (typeof this.service.amsNetId !== 'string' ||
            ...
            try {
                ...
                if (tcVersion === '2') {
                    this.serviceInfo.alignment = parseInt(configFile.getElementsByTagName('PackSize')[0].childNodes[0]
                        .nodeValue, 10);
                }
                else if (tcVersion === '3') {
                ...
                for (let i = 0; i < symbolArray.length; i++) {
                    ...
                    this.symTable[name] = {
                        typeString: symbolArray[i]
                            ...
                            .childNodes[0].nodeValue.toUpperCase(),
                        indexGroup: parseInt(symbolArray[i].getElementsByTagName('IGroup')[0].childNodes[0]
                            .nodeValue, 10),
                        indexOffset: parseInt(symbolArray[i].getElementsByTagName('IOffset')[0].childNodes[0]
                            .nodeValue, 10),
                        bitSize: parseInt(symbolArray[i].getElementsByTagName('BitSize')[0].childNodes[0]
                            .nodeValue, 10),
                    };
                ...
                for (let i = 0; i < dataTypeArray.length; i++) {
                    ...
                    if (name === 'ARRAY') {
                        this.dataTypeTable[fullName] = {
                            ...
                            bitSize: parseInt(dataTypeArray[i].getElementsByTagName('BitSize')[0]
                                .childNodes[0].nodeValue, 10),
                        };
                    ...
                    if (name !== 'ARRAY') {
                        ...
                        for (let j = 0; j < subItemArray.length; j++) {
                            ...
                            this.dataTypeTable[name].subItems[sName] = {
                                ...
                                bitSize: parseInt(subItemArray[j].getElementsByTagName('BitSize')[0]
                                    .childNodes[0].nodeValue, 10),
                            };
                            if (subItemArray[j].getElementsByTagName('BitOffs')[0] !== undefined) {
                                this.dataTypeTable[name].subItems[sName].bitOffset = parseInt(subItemArray[j].getElementsByTagName('BitOffs')[0]
                                    .childNodes[0].nodeValue, 10);
                            }
                            ...
                            if (typeArr[0] === 'ARRAY') {
                                ...
                                this.dataTypeTable[name].subItems[sName].bitSize =
                                    this.dataTypeTable[this.dataTypeTable[name].subItems[sName].typeString].bitSize;
                                this.dataTypeTable[name].subItems[sName].size =
                                    this.dataTypeTable[this.dataTypeTable[name].subItems[sName].typeString].size;
                                //Item length
                                ...
                                if (this.dataTypeTable[name].subItems[sName].arrayDataType ===
                                    'USER') {
    ...
    setServiceParamFromTPY() {
        ...
        log('TAME library info: Target information: NetId: ' +
