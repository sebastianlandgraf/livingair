 ...
 */
export function getTypeAndFormat(item) {
    let arr = [];
 ...
 */
export function parseVarName(name, data, obj, prefix, suffix) {
    let arr = [];
    let last = 0;
    let i = 0;
 ...
 * @param {Number} len
 */
export function isValidStringLen(len) {
    if (len === undefined) {
        return false;
    }
...
function checkValue(item, type, min, max) {
    ...
    if (typeof item.val === 'string') {
        ...
        log('TAME library error: Variable type should be number or string, but is ' +
            typeof item.val);
        log(item);
        val = 0;
    }
 ...
 * @param {String} format
 */
export function toMillisec(time, format) {
    let tmp;
    switch (format) {
        case '#d':
        case '#dd':
 ...
 * @param {Number} varlen
 */
export function numToByteArr(value, varlen) {
    const bytes = [];
 ...
 * @param {Number} value
 */
export function numToHexString(value) {
 ...
 * @param {Number} places
 */
export function fixNumbLength(numb, places) {
 ...
 */
export function charcodeToDual(charcode) {
 ...
 * @param {Number} num
 */
export function floatToReal(num) {
    let mant = 0, real = 0, bas = 0, bas2, tmp, exp = 0;
 ...
 * @param {Number} num
 */
export function floatToLreal(num) {
    let mant = 0, mant2 = 0;
    const lreal = {
        part1: 0,
        part2: 0,
    };
    let tmp, exp = 0, firstbit, bas = 0;
    const abs = Math.abs(num);
    if (num !== 0) {
        //Find exponent and base.
 ...
 * @param {String} format
 */
export function stringToTime(timestring, format) {
    const arrFormat = format.split('#');
    const arrlen = arrFormat.length;
    const regex = /:|\.|-|_/;
    let time = 0, cnt = 0, tmp;
 ...
 */
export function dataToByteArray(item, type, format, len) {
    let bytes = [], strlen, sl, i;
